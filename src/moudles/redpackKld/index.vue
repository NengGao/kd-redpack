<template>
	<div class="hot flex-wrap">
		...
		<ul class="kld-room flex-wrap">
			<li :class='{"active" : kldRoomType == 50}' @click="changeRoomType(50)">50豆</li>
			<li :class='{"active" : kldRoomType == 100}' @click="changeRoomType(100)">100豆</li>
			<li :class='{"active" : kldRoomType == 200}' @click="changeRoomType(200)">200豆</li>
		</ul>
		...
		<div class="footer flex-wrap" id="footer">
			<div class="user-balance">
				<p>我的快乐豆</p>
				<div class="balance-box">{{user.balance | KLD_f}}<i class="ic-add" @click="getKld()"></i></div>
			</div>
			<ul class="games-function">
				...
				<router-link :to='"/shop"' tag="li"><i class="ic-shop"></i></router-link>
			</ul>
		</div>
	...
	export default {
		data() {
			return {
				...
				roomid : '',
				kldRoomType: common.getLocal("kldRoomType"),
				redpackObj : {
					'_0' : {
						fromUserImg : 'imgurl',
						fromUserName : '大富翁'
				...
				paySet:{
					payPrice : 10,
					type : 'DDJL',
					returnUrl : 'http://' + config.host + encodeURIComponent('/#/redpackKld/index'),
					balance : true,
	        		balancePay : ''
				},
				redpackState :{
		...
		created() {
			...
			if(!this.kldRoomType){
				this.kldRoomType = '100';
			}
			this.enterTypeRoom(this.kldRoomType);
		},
		methods: {
			//刷新 滚动条
			_initScroll() {
			...
			changeRoomType(roomType){
				this.kldRoomType = roomType;
				common.setLocal("kldRoomType",roomType);
				this.enterTypeRoom(roomType);
			},
			enterTypeRoom(roomType){
				roomType = 'DDJL' + roomType;
				let self = this;
				let intoRoom = {
					msgType : 3,
			...
			kdPay(){
				...
				this.paySet.payPrice = this.Counter.init;
...
<style rel="stylesheet/scss" lang="scss" scoped>
	@import "../../assets/sass/redpackKld/redpackKld.scss";
</style>